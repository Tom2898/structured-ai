...
IMPORTANTE: Usa esclusivamente dati trovati online. Se non trovi informazioni per alcune sezioni, indicalo chiaramente con "Dato non disponibile".`;
      ...
      try {
        const promptUnderlyings = `Analizza i sottostanti del certificato ISIN ${isin}.\n${cedContext}\nDalla scheda sopra estrai i sottostanti con i loro strike. Per ogni sottostante:\n1. Cerca online il prezzo spot attuale\n2. La barriera e' tipicamente il 60-70% dello strike per un Phoenix Memory - calcolala come strike * 0.60 se non disponibile online\n3. Calcola distanzaBarriera = (spot - barriera) / spot * 100\nRispondi SOLO con questo JSON, nessun testo fuori:\n{"underlyings":[{"ticker":"<TICKER>","name":"<nome completo>","spot":0.00,"strike":0.00,"barriera":0.00,"distanzaBarriera":0.00,"var1Y":0.00,"trend":"bullish|bearish|neutro","note":"<commento breve su situazione attuale>"}]}`;
              ...
              <div style={{ maxWidth: 720, margin: "0 auto" }}>
                ...
                {isinAnalysisResult && !isinAnalysisLoading && (
                  <div style={{ background:"var(--surface)", border:"1px solid var(--border)", borderRadius:"var(--radius)", padding:"2rem" }}>
                    ...
                    {/* Sezione 2 - Analisi Sottostanti */}
                    {isinAnalysisUnderlyings && isinAnalysisUnderlyings.length > 0 && (
                      <div style={{ marginTop:24, background:"var(--surface)", border:"1px solid var(--border)", borderRadius:"var(--radius)", padding:"2rem" }}>
